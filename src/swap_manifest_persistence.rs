...
//!
//! The fully validated migration-050/051/053 chain-swap row is committed first so
...
use crate::db::{
    ...
    record_chain_swap_with_lineage_and_creation_evidence, ChainSwapLineage, ChainSwapRecord,
    ManifestDeliveryError, ManifestDeliveryIdentity, NewChainSwapCreationEvidence,
    NewChainSwapCreationTerms, NewChainSwapRecord,
};
...
pub enum ManifestlessChainSwapRepairError {
    ...
    MerchantNymMissing,
    EmergencyPolicyReferenceIncomplete,
    ManifestEvidenceReadFailed,
    ManifestEvidenceMissing,
    ManifestStagingFailed,
    ManifestIdentityFailed,
...
impl fmt::Display for ManifestlessChainSwapRepairError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            ...
            Self::EmergencyPolicyReferenceIncomplete => {
...
fn exact_emergency_policy_reference(
    commitment_id: Option<Uuid>,
    address: Option<&str>,
) -> Result<Option<(Uuid, &str)>, ManifestlessChainSwapRepairError> {
    match (commitment_id, address) {
        (Some(commitment_id), Some(address)) => Ok(Some((commitment_id, address))),
        (None, None) => Ok(None),
        _ => Err(ManifestlessChainSwapRepairError::EmergencyPolicyReferenceIncomplete),
    }
}
...
pub async fn repair_oldest_manifestless_chain_swap(
    ...
    let emergency_bitcoin = match exact_emergency_policy_reference(
        creation_terms.recovery_address_commitment_id,
        creation_terms.merchant_emergency_btc_address.as_deref(),
    ) {
...
mod manifestless_repair_error_tests {
    ...
    fn emergency_policy_reconstruction_preserves_exact_pair_and_legacy_absence() {
        let commitment_id = uuid::Uuid::from_u128(0x84);
        let address = "bc1p0xlxvlhemja6c4dqv22uapctqupfhlxm9h8z3k2e72q4k9hcz7vqzk5jj0";
        assert_eq!(
            exact_emergency_policy_reference(Some(commitment_id), Some(address)),
            Ok(Some((commitment_id, address)))
        );
        assert_eq!(exact_emergency_policy_reference(None, None), Ok(None));
    }

    #[test]
    fn emergency_policy_reconstruction_rejects_only_asymmetric_evidence() {
        let commitment_id = uuid::Uuid::from_u128(0x84);
        assert_eq!(
            exact_emergency_policy_reference(Some(commitment_id), None),
            Err(Error::EmergencyPolicyReferenceIncomplete)
        );
        assert_eq!(
            exact_emergency_policy_reference(None, Some("bc1qpolicy")),
            Err(Error::EmergencyPolicyReferenceIncomplete)
        );
    }
    ...
    fn failures_are_bounded_and_source_free() {
        for error in [
            ...
            Error::EmergencyPolicyReferenceIncomplete,
...
pub async fn persist_and_deliver_chain_swap(
    ...
    let creation_evidence = NewChainSwapCreationEvidence {
        creation_terms: *creation_terms,
        recovery_address_commitment_id: merchant_policy.emergency_bitcoin_commitment_id,
    };
    let chain_swap = record_chain_swap_with_lineage_and_creation_evidence(
        pool,
        swap,
        lineage,
        &creation_evidence,
