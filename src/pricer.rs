...
mod tests {
    ...
    fn json_rpc_response_decodes_pricer_shape() {
        // Real shape from api.bullbitcoin.com/public/price (verified
        // 2026-05-07 against prod): no `marketPrice`, only `price` +
        // `indexPrice`. We use `indexPrice` (neutral reference) for
        // donation-page rendering.
        let raw = r#"{
            "jsonrpc": "2.0",
            "id": 1,
            "result": {
