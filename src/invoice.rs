...
/// POST /a/:slug/invoice — keyless Payment Page checkout selected by the
/// owner's shared alias.
...
/// POST /a/:slug/pos/invoice — keyless POS checkout selected by the same
/// npub-level alias as the Payment Page.
...
async fn create_anonymous_alias_for_kind(
    ...
    if !page.alias_active || !page.enabled || page.is_archived {
