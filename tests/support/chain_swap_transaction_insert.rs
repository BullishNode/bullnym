...
use pay_service::db::{
    ChainSwapCreationTerms, ChainSwapLineage, ChainSwapRecord, NewChainSwapCreationTerms,
    NewChainSwapRecord, NewSwapKeyAllocation, SwapKeyPurpose, DERIVATION_SCHEME_VERSION,
};

use super::{assert_sqlstate, cleanup_db, create_test_user, insert_test_invoice, test_pool};
...
fn assert_record_matches_input(
    ...
    assert_eq!(
        ...
        Some(&ChainSwapCreationTerms::from(creation_terms))
    );
...
async fn fully_validated_chain_swap_insert_in_tx_has_pool_parity_and_commit_visibility() {
    ...
    let creation_terms = fixture.creation_terms();
    let inserted = pay_service::db::record_chain_swap_with_lineage_and_creation_terms_in_tx(
        &mut tx,
        &tx_swap,
        &tx_lineage,
        &creation_terms,
    ...
    .await
    .unwrap();
    assert_record_matches_input(&inserted, &tx_swap, &creation_terms);
    ...
    let pool_inserted = pay_service::db::record_chain_swap_with_lineage_and_creation_terms(
        &pool,
        &pool_swap,
        &pool_lineage,
        &creation_terms,
    ...
    .await
    .unwrap();
    assert_record_matches_input(&pool_inserted, &pool_swap, &creation_terms);
...
async fn fully_validated_chain_swap_insert_in_tx_rolls_back_and_keeps_constraints() {
    ...
    let creation_terms = fixture.creation_terms();
    let rolled_back = pay_service::db::record_chain_swap_with_lineage_and_creation_terms_in_tx(
        &mut rollback_tx,
        &rollback_swap,
        &rollback_lineage,
        &creation_terms,
    ...
    let mut invalid_terms = fixture.creation_terms();
    invalid_terms.pinned_pair_hash = "not-a-lowercase-sha256";
    let invalid = pay_service::db::record_chain_swap_with_lineage_and_creation_terms_in_tx(
        &mut invalid_tx,
        &invalid_swap,
        &invalid_lineage,
        &invalid_terms,
