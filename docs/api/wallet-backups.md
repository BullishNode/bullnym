...
Errors generated by Bullnym use its JSON envelope and a meaningful HTTP
status:
...
All Bullnym-generated success and error responses set `Cache-Control: private,
no-store, max-age=0` and `Pragma: no-cache` so authenticated backup payloads
and heads are not retained by ordinary browser or intermediary caches. A
reverse proxy can reject a request before Bullnym sees it, including `413` at
its body ceiling and `429` at its independent burst gate. Clients must classify
those responses by HTTP status and must not depend on a Bullnym JSON body.
