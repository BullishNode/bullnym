...
Aliases use the same 1-32 character lowercase letter, digit, and internal
hyphen syntax as nyms. Leading/trailing hyphens, route names,
signing-confusion values (`0`, `1`, `pos`), and reserved Bull Bitcoin brand
names are rejected. Mirror these checks client-side for feedback, but treat
the server as authoritative.

`NameTaken` with HTTP 409 means the string is already permanently reserved as
either a nym or alias. `AliasAlreadyAssigned` with HTTP 409 means this npub
already owns a different lifetime alias; offer reactivation of that alias
instead of suggesting another name. Omitting `alias` preserves state and its
older signed-byte layout; an empty alias is present and must be signed as the
terminal field.
