...
| `donation_pages` | Public payment surfaces | One row per `(nym, kind)`, where `kind` is `payment_page` or `pos`. Stores display content, legacy read-only media hashes, descriptor, address cursor, and archive state. Aliases resolve through `public_names`. |
