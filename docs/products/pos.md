...
POS is a public terminal surface at `https://<domain>/<nym>/pos` or, when the
owner has an active alias, `https://<domain>/a/<alias>/pos`. It uses the same
payment-session engine as Payment Pages, but it has a separate `donation_pages`
row with `kind = 'pos'`.
...
| Item | Value |
|---|---|
| Public shell | `GET /:nym/pos` or `GET /a/:alias/pos` |
| Manifest | `GET /:nym/pos/manifest.webmanifest` or `GET /a/:alias/pos/manifest.webmanifest` |
| Invoice creation | `POST /:nym/pos/invoice` or `POST /a/:alias/pos/invoice` |
...
The alias belongs to the npub and is shared with Payment Page. If no alias is
active, generated POS links use the nym route without storing a synthetic
alias. Nym routes remain valid after an alias is claimed.
