...
DECLARE
    ...
    existing_checkpoint_version BIGINT;
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        -- PostgreSQL executes BEFORE INSERT triggers before resolving an
        -- INSERT .. ON CONFLICT DO UPDATE.  Distinguish a true first write
        -- from the insert candidate for a CAS advance; the UPDATE arm below
        -- still enforces immutable identity and an exact +1 transition.
        SELECT checkpoint_version
          INTO existing_checkpoint_version
          FROM merchant_settlement_checkpoints
         WHERE chain_swap_id = NEW.chain_swap_id
           AND settlement_path = NEW.settlement_path;
        IF NOT FOUND AND NEW.checkpoint_version <> 1 THEN
            RAISE EXCEPTION 'merchant settlement checkpoint must start at version 1'
                USING ERRCODE = '23514';
        ELSIF FOUND
              AND NEW.checkpoint_version <> existing_checkpoint_version + 1 THEN
            RAISE EXCEPTION 'merchant settlement checkpoint insert candidate must advance one version'
                USING ERRCODE = '55000';
        END IF;
        RETURN NEW;
    END IF;
